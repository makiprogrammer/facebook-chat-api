...
function formatThreadList(data) {
	return data.map(t => {
		let lastMessageNode =
			t.last_message && t.last_message.nodes && t.last_message.nodes.length > 0 ? t.last_message.nodes[0] : null;
		return {
			...
			snippetSender: lastMessageNode
				? utils.formatID((lastMessageNode.message_sender.messaging_actor.id || '').toString())
				: null,
